  ...
  <div class="matchpanel">
    ...
    <div class="field-row">
      ...
      <div class="field">
        ...
        <input id="mp-household" type="number" min="1" max="8" inputmode="numeric" v-model.number="profile.household" />
      </div>
    </div>
    ...
    <fieldset class="checks">
      ...
      <label><input type="checkbox" v-model="profile.parentSupport" /> 만 65세 이상 직계존속을 3년 이상 부양 중이다</label>
    </fieldset>

    <div class="matchpanel__result" :class="{ 'is-off': !ui.matchOn }">
      <template v-if="ui.matchOn">
        ...
        <ul v-if="topBlockers.length" class="matchpanel__blockers">
          <li v-for="[reason, n] in topBlockers" :key="reason">{{ reason }} <span>{{ n }}건</span></li>
        </ul>
      </template>
      <p v-else class="muted">조건을 켜면 40건을 한 번에 훑어 자격을 표시합니다.</p>
      <button type="button" class="linkish" @click="reset">기본값으로 되돌리기</button>
    </div>

    <p class="notice-tip">
      자격 판단은 공고문에서 뽑아낸 요약값 기준이라 실제 심사와 다를 수 있어요. 기관추천처럼 자동으로 확인할 수 없는
      유형은 <b>확인 필요</b>로 표시합니다. 청약 전에는 반드시 청약홈과 원문 공고를 확인하세요.
    </p>
  </div>
</template>
